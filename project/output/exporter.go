...
func NewBaseExporter(p *project.Project, collector ContentCollector) *BaseExporter {
	return &BaseExporter{
		project:   p,
		collector: collector,
	}
...
func (b *BaseExporter) Export(outputPath string) error {
	if b.project.IsEmpty() {
		err := b.collector.AddTitle("空项目", 1)
		if err != nil {
			return err
		}
	...
	traverser.SetTraverseOrder(project.PreOrder).TraverseTree(b)
	return b.collector.Render(outputPath)
}
